...
function bindCard(w, book) {
  ...
  w.addEventListener(event.CLICK_DOWN, (function (b) {
    return function () {
      // 滑动翻页后的误触：滑动抬起后 400ms 内的按下不启动长按（避免弹删除框）
      if (Date.now() - lastSwipeTs < 400) return
      lp = false
...
function renderLaunchButton() {
  clearLaunch()
  var enabled = launchCalcEnabled()
  var w = sp(220), h = sp(40), x = Math.round((W - w) / 2), y = sp(406)
...
function showDeleteConfirm(book) {
  ...
  // 背景吸收点击，避免误触下层书卡
  // 背景/面板直接显示（不做依赖定时器的淡入：本设备定时器回调不可靠会卡在透明态）
...
function drawNav(totalPages) {
  var y = sp(336), bh = sp(44), bw = sp(68)
  addShelfWidget(createWidget(widget.TEXT, {
...
function renderShelf() {
  ...
  // 单列，每页 3 张：106 / 184 / 262（卡底 332），翻页控件 336（书卡下移避开顶部按钮圆角）
  var gy = sp(106)
...
// 左右滑动翻页：与箭头按钮同一逻辑（dir>0 下一页）
var _swipeLastTs = 0   // 手势节流时间戳
// 最近一次滑动手势时间：bindCard 据此忽略滑动造成的误触（按下/抬起会被当作点击）
var lastSwipeTs = 0
function shelfSwipe(dir) {
  lastSwipeTs = Date.now()
  var totalPages = Math.max(1, Math.ceil(getAllBooks().length / PAGE_SIZE))
  if (dir > 0 && shelfPage < totalPages - 1) { shelfPage++; renderShelf() }
  else if (dir < 0 && shelfPage > 0) { shelfPage--; renderShelf() }
}
...
function toast(text) {
  ...
  // 系统风格底部胶囊提示
  // 直接显示、到时直接删除，不做动画
  toastWidgets.push(createWidget(widget.FILL_RECT, { x: sp(100), y: sp(376), w: sp(280), h: sp(48), radius: sp(24), color: COL_PANEL_SOFT }))
  toastWidgets.push(createWidget(widget.TEXT, { x: sp(112), y: sp(376), w: sp(256), h: sp(48), text: text, text_size: sp(M.tsVal), color: COL_TEXT, align_h: align.CENTER_H, align_v: align.CENTER_V }))
...
Page({
  build() {
    ...
    // 顶部三个功能钮：FILL_RECT 拼图标 + 文字胶囊（emoji 在 TEXT 控件会滚动错位，改用几何图标）
    var _bw = sp(96), _bh = sp(42), _by = sp(58)
    var _bgap = sp(8)
    var _bx0 = Math.round((W - (3 * _bw + 2 * _bgap)) / 2)
    var _labels = [
      { t: getText('shelfBtnPwd'), ic: 0, f: function () { showPwdPanel() } },
      { t: getText('shelfBtnStats'), ic: 1, f: function () { showStats() } },
      { t: getText('shelfBtnAbout'), ic: 2, f: function () { showAbout() } }
    ]
    // 按钮内 26×26 图标：0 锁 / 1 柱状图 / 2 信息
    function homeIcon(x, y, ic) {
      var A = COL_ACCENT_SOFT, B = COL_PANEL
      if (ic === 0) {                       // 挂锁：外圆+内圆挖空成环，锁体盖住环下半 → U 形锁梁
        createWidget(widget.FILL_RECT, { x: x + sp(3), y: y + sp(1), w: sp(20), h: sp(20), radius: sp(10), color: A })   // 外圆
        createWidget(widget.FILL_RECT, { x: x + sp(7), y: y + sp(5), w: sp(12), h: sp(12), radius: sp(6), color: B })   // 内圆挖空（用按钮底色）
        createWidget(widget.FILL_RECT, { x: x + sp(3), y: y + sp(12), w: sp(20), h: sp(12), radius: sp(3), color: A })   // 锁体盖住环下半
        createWidget(widget.FILL_RECT, { x: x + sp(10), y: y + sp(15), w: sp(6), h: sp(7), radius: sp(2), color: B })   // 锁孔
      } else if (ic === 1) {                // 柱状图
        createWidget(widget.FILL_RECT, { x: x + sp(3), y: y + sp(9), w: sp(5), h: sp(13), radius: sp(2), color: A })
        createWidget(widget.FILL_RECT, { x: x + sp(10), y: y + sp(5), w: sp(5), h: sp(17), radius: sp(2), color: A })
        createWidget(widget.FILL_RECT, { x: x + sp(17), y: y + sp(1), w: sp(5), h: sp(21), radius: sp(2), color: A })
      } else {                              // 信息 i
        createWidget(widget.FILL_RECT, { x: x + sp(3), y: y + sp(3), w: sp(20), h: sp(20), radius: sp(10), color: A })
        createWidget(widget.FILL_RECT, { x: x + sp(11), y: y + sp(7), w: sp(3), h: sp(8), radius: sp(1), color: B })
        createWidget(widget.FILL_RECT, { x: x + sp(11), y: y + sp(16), w: sp(3), h: sp(3), radius: sp(1), color: B })
      }
    }
    for (var _bi = 0; _bi < 3; _bi++) {
      ...
      // 左侧图标 26×26 + 右侧文字
      homeIcon(_bx + sp(6), _by + Math.round((_bh - sp(24)) / 2) - sp(1), _labels[_bi].ic)
      createWidget(widget.TEXT, { x: _bx + sp(34), y: _by, w: _bw - sp(40), h: _bh, text: _labels[_bi].t, text_size: sp(M.tsMeta), color: COL_ACCENT_SOFT, align_h: align.CENTER_H, align_v: align.CENTER_V })
    ...
    renderLaunchButton()
    startPoll()
    // 左右滑动翻页（返回 true 消费手势，防止与系统返回/退出手势冲突）
    try {
      offGesture()
      onGesture({
        callback: function (e) {
          if (recvShown || !shelfAlive) return false
          if (e === GESTURE_LEFT || e === GESTURE_RIGHT) {
            // 300ms 节流（参考 RingNotes 同款实现），防止连滑触发多次重建
            var now = Date.now()
            if (now - _swipeLastTs >= 300) {
              _swipeLastTs = now
              shelfSwipe(e === GESTURE_LEFT ? 1 : -1)
            }
            return true   // 始终拦截，防止系统接管（上滑=退出等）
          }
          return false
        }
      })
