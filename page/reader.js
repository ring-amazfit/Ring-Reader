...
// ── 菜单：单屏布局，全部内容一屏放完 ──
// 行1 三联开关（全屏/滚动/常亮）
// 行2 表冠灵敏度（滑条 + 右侧角度阈值，如 60°）
// 行3 [自动] [书签] 两列大卡
// 行4 [跳页] [样式] 两列大卡
// 卡片高度 70、宽度按圆屏可用宽取半（尽量大），底部关闭大圆钮。
var MROW = 48                       // 行高
var CROWN_H = 60                    // 表冠行高（加大，更好操作）
var CARD_H = 60                     // 两列卡片高
...
/**
 * 两列大卡片：label 居中 + 右下角 sub（值 / 箭头）。整卡可点。
 * 返回 { setSub(text) } 用于刷新值（自动翻页模式等）。
 */
// 卡片左上角图标：TEXT 控件放 emoji 会滚动/错位（代理对字符被当多字符），
// 改用 FILL_RECT 拼图标，位置精确、任何设备一致。
// icon: 0 时钟(自动) / 1 书(书签) / 2 井号(跳页) / 3 调色盘(样式)
function cardIcon(x, y, icon) {
  var A = C.accent, B = C.card
  if (icon === 0) {                       // 时钟：自动翻页计时
    mAdd(createWidget(widget.FILL_RECT, { x: x + 1, y: y + 1, w: 20, h: 20, radius: 10, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 4, y: y + 4, w: 14, h: 14, radius: 7, color: B }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 11, y: y + 5, w: 2, h: 7, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 11, y: y + 11, w: 6, h: 2, color: A }))
  } else if (icon === 1) {                // 两本书
    mAdd(createWidget(widget.FILL_RECT, { x: x + 1, y: y + 5, w: 8, h: 14, radius: 2, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 4, y: y + 7, w: 2, h: 10, color: B }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 11, y: y + 3, w: 8, h: 14, radius: 2, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 14, y: y + 5, w: 2, h: 10, color: B }))
  } else if (icon === 2) {                // 井号：跳页数字（4px 粗，横竖中心对称）
    mAdd(createWidget(widget.FILL_RECT, { x: x + 2, y: y + 6, w: 18, h: 4, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 2, y: y + 12, w: 18, h: 4, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 6, y: y + 2, w: 4, h: 18, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 12, y: y + 2, w: 4, h: 18, color: A }))
  } else {                                // 文本行：三条横线居中对称（中心都在 11，无竖线）
    mAdd(createWidget(widget.FILL_RECT, { x: x + 2, y: y + 3, w: 18, h: 3, radius: 1, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 5, y: y + 10, w: 12, h: 3, radius: 1, color: A }))
    mAdd(createWidget(widget.FILL_RECT, { x: x + 2, y: y + 17, w: 18, h: 3, radius: 1, color: A }))
  }
}

function menuCard(x, y, w, h, label, sub, subIsValue, icon, onClick) {
  mAdd(createWidget(widget.FILL_RECT, { x: x, y: y, w: w, h: h, radius: 18, color: C.card }))
  if (icon !== undefined && icon !== null) cardIcon(x + 10, y + 9, icon)
  mAdd(createWidget(widget.TEXT, { x: x, y: y + 6, w: w, h: 26, text: label, text_size: M.tsRow, color: C.text, align_h: align.CENTER_H, align_v: align.CENTER_V }))
  var st = mAdd(createWidget(widget.TEXT, {
    x: x + 12, y: y + h - 22, w: w - 24, h: 18, text: sub || '', text_size: M.tsVal,
    color: subIsValue ? C.accent : C.muted,
    align_h: subIsValue ? align.CENTER_H : align.RIGHT, align_v: align.CENTER_V
  }))
  var t = mAdd(createWidget(widget.FILL_RECT, { x: x, y: y, w: w, h: h, radius: 18, color: 0x000000, alpha: 0 }))
  t.addEventListener(event.CLICK_DOWN, onClick)
  return { setSub: function (s, isVal) {
    try { st.setProperty(prop.MORE, { text: s, color: isVal ? C.accent : C.muted, align_h: isVal ? align.CENTER_H : align.RIGHT }) } catch (e) {}
  } }
}

/**
 * 三联开关 chip（全屏 / 滚动 / 常亮），开启态 = 暖色底 + 琥珀文字。
 */
...
// 表冠灵敏度行：label + 自绘滑条 + 右侧角度阈值（90°/80°/60°/40°/20°）
function menuCrownRow(y) {
  var g = menuRowGeo(y, CROWN_H)
  mAdd(createWidget(widget.FILL_RECT, { x: g.x, y: y, w: g.w, h: CROWN_H, radius: Math.round(CROWN_H / 2), color: C.card }))
  mAdd(createWidget(widget.TEXT, { x: g.x + M.padX, y: y, w: 80, h: CROWN_H, text: getText('readerCrown'), text_size: M.tsRow, color: C.text, align_v: align.CENTER_V }))
  var crownLevel = 3
  try {
    var cv = parseInt(localStorage.getItem('crown_level') || '', 10)
    if (cv >= 1 && cv <= 5) crownLevel = cv
    else {
      var co = parseInt(localStorage.getItem('crown_step') || '', 10)
      if (co >= 1 && co <= 3) crownLevel = co
    }
  } catch (e) {}
  var degText = mAdd(createWidget(widget.TEXT, { x: g.x + g.w - M.padX - 44, y: y, w: 44, h: CROWN_H, text: CROWN_LEVELS[crownLevel - 1] + '°', text_size: M.tsVal, color: C.accent, align_h: align.RIGHT, align_v: align.CENTER_V }))
  var slX = g.x + M.padX + 88
  var slW = (g.x + g.w - M.padX - 54) - slX
  makeSlider(UIDEPS, mAdd, slX, y + Math.round((CROWN_H - M.sliderKnob) / 2), slW, 1, 5, crownLevel,
    function (v) { try { degText.setProperty(prop.TEXT, CROWN_LEVELS[v - 1] + '°') } catch (e) {} },
...
function openMenu() {
  ...
  // 头部：大号百分比 + 页码/时间 + 时长（数据行放大，小字可读）
  mAdd(createWidget(widget.TEXT, { x: 90, y: 18, w: W - 180, h: 36, text: percent() + '%', text_size: 32, color: C.accent, align_h: align.CENTER_H, align_v: align.CENTER_V }))
  mAdd(createWidget(widget.TEXT, { x: 60, y: 54, w: W - 120, h: 22, text: displayPage() + ' / ~' + estTotal + '   ' + nowHHMM(), text_size: 16, color: C.sub, align_h: align.CENTER_H }))
  menu.timerText = mAdd(createWidget(widget.TEXT, { x: 60, y: 76, w: W - 120, h: 18, text: getText('readerSessionTime') + fmtMin(currentReadSec() - baseReadSec) + ' · ' + getText('readerTotalTime') + fmtMin(currentReadSec()), text_size: 13, color: C.muted, align_h: align.CENTER_H }))

  // 行1：三联开关
  menuToggleChips(102)

  // 行2：表冠灵敏度（滑条 + 角度阈值，行高 60）
  menuCrownRow(158)

  // 行3/4：两列大卡片 [自动][书签] / [跳页][样式]，高 60，宽度按圆屏可用宽尽量取大
  function twoCards(y) {
    var w = Math.min(MCAP, rowW(y, CARD_H))
    var gap = 12
    var cw = Math.floor((w - gap) / 2)
    var x0 = Math.round((W - w) / 2)
    return { x0: x0, cw: cw, gap: gap }
  }
  var r1 = twoCards(226)
  // 自动（值卡片，图标=时钟）
  var autoCard = menuCard(r1.x0, 226, r1.cw, CARD_H, getText('readerAuto'), getText(AUTO_LABELS[autoIdx]), true, 0, function () { changeAuto(1) })
  menu.autoBtnTxt = autoCard
  // 书签（图标=书）
  menuCard(r1.x0 + r1.cw + r1.gap, 226, r1.cw, CARD_H, getText('readerBookmarks'), '>', false, 1, function () { _backToMenu = true; closeMenu(); openBookmarks() })

  var r2 = twoCards(294)
  // 跳页（图标=井号）
  menuCard(r2.x0, 294, r2.cw, CARD_H, getText('readerJump'), '>', false, 2, function () { _backToMenu = true; closeMenu(); openJumpPanel() })
  // 样式（图标=调色盘）
  menuCard(r2.x0 + r2.cw + r2.gap, 294, r2.cw, CARD_H, getText('readerStyle'), '>', false, 3, function () { _backToMenu = true; closeMenu(); openStylePage() })

  // 关闭按钮：底部居中大圆钮
  makeCloseButton(UIDEPS, mAdd, H - 54, function () { closeMenu() }, W)
...
function openJumpPanel() {
  ...
  // 模式切换 chip（改小：104×30，页数/百分比切换不占版面）
  var chipW = 104, chipH2 = 30, chipX = Math.round((W - chipW) / 2), chipY = 96
  jAdd(createWidget(widget.FILL_RECT, { x: chipX, y: chipY, w: chipW, h: chipH2, radius: Math.round(chipH2 / 2), color: C.cardAlt }))
  var chipTxt = jAdd(createWidget(widget.TEXT, { x: chipX, y: chipY, w: chipW, h: chipH2, text: '', text_size: M.tsMeta, color: C.accentSoft, align_h: align.CENTER_H, align_v: align.CENTER_V }))
  var modeTouch = jAdd(createWidget(widget.FILL_RECT, { x: chipX - 8, y: chipY - 6, w: chipW + 16, h: chipH2 + 12, radius: Math.round(chipH2 / 2) + 6, color: 0x000000, alpha: 0 }))
  function paintMode() {
